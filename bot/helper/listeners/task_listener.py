...
class TaskListener(TaskConfig):
    ...
    async def on_download_start(self):
        # Start elapsed-time timer for the complete task.
        # monotonic() is used so system clock changes do not affect timing.
    ...
    async def on_download_complete(self):
        ...
        if self.is_leech:
            ...
            LOGGER.info(
                f"BuzzHeavier Upload Name: {self.name}"
            )

            bh = BuzzHeavierUploader(
                self,
                up_path,
            )

            async with task_dict_lock:
                task_dict[self.mid] = BuzzHeavierStatus(
                    self,
                    bh,
                    gid,
                    "up",
            ...
            LOGGER.info(
                f"GoFile Upload Name: {self.name}"
            )
    ...
    async def on_upload_complete(
        ...
        # Calculate complete elapsed time.
        if self.task_start_time is not None:
            elapsed_seconds = int(
                time.monotonic() - self.task_start_time
            )
        else:
            elapsed_seconds = 0
        ...
        if self.is_leech:
            ...
            if self.is_buzzheavier:
                ...
                # Google Drive:
                # ONLY Index Link + View Link for media
                buttons = ButtonMaker()
                ...
                if INDEX_URL and dir_id:
                    share_url = (
                        f"{INDEX_URL}"
                        f"findpath?id={dir_id}"
                    )
                ...
                if (
                    ...
                    remote, rpath = rclone_path.split(
                        ":",
                        1,
                    )
    ...
    async def on_download_error(
        ...
        torbox_torrent_id = getattr(
            self,
            "_torbox_torrent_id",
            0,
        ) or 0

        torbox_web_id = getattr(
            self,
            "_torbox_web_id",
            0,
        ) or 0
